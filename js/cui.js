...
(function() {
  ...
  window.__cui = {
    ...
    "patch": "1",
    "full": "2.5.1",
    "releaseDate": "9/16/2014"
  };

  cui = angular.module('cui', ["cui.controls.alert", "cui.controls.badge", "cui.controls.breadcrumb", "cui.controls.button", "cui.controls.calendar", "cui.controls.checkbox", "cui.controls.combobox", "cui.controls.dataGrid", "cui.controls.datePicker", "cui.controls.dropDownButton", "cui.controls.dropDownList", "cui.controls.icon", "cui.controls.masthead", "cui.controls.memo", "cui.controls.menu", "cui.controls.navigationList", "cui.controls.pane", "cui.controls.progressBar", "cui.controls.radio", "cui.controls.richTextEditor", "cui.controls.spinner", "cui.controls.splitButton", "cui.controls.table", "cui.controls.tabset", "cui.controls.textarea", "cui.controls.textbox", "cui.controls.tooltip", "cui.controls.tree", "cui.modules.aboutBox", "cui.modules.applicationFrame", "cui.modules.startScreen", "cui.modules.wizard", "cui.modules.wizard", "cui.services.alertService", "cui.services.dataSource", "cui.services.dialog", "cui.services.loading", "cui.services.modal", "cui.services.modal", "cui.services.modal", "cui.services.modal", "cui.services.position"]);

  angular.module('cui.base', ['cui.templates', 'smartTable.table', 'pascalprecht.translate']).value('baseTemplatePath', '__cui/').config(function($translateProvider) {
  ...
  Messages and links for additional information should be displayed on the left. Action buttons should be displayed on the right.
    ...
    @param {boolean=} dismissable Whether to show the button to manually close the alert. Defaults to `true`.
  ...
  ** Other: `'ok-sign'`
  ...
  module.directive('cuiAlert', function(baseTemplatePath, $timeout) {
    ...
    return {
      ...
      link: function(scope, element, attrs) {
        ...
        scope.close = function() {
          scope.$emit('cuiAlert:destroy');
          element.addClass('cui-zero-height');
        ...
        $timeout((function() {
          ...
          return $timeout((function() {
            ...
            return $timeout((function() {
              ...
              return element.removeClass('cui-zero-height');
            }), TRANSITION_TIME);
          }), 20);
        }), 0);
      }
  ...
  /**
    ...
    @restrict E
    @param {expression} items An array of objects with the following structure:
  ...
  > The Calendar is primarily used with the Date Picker.
    ...
    @controlType input
    @restrict E
    @param {Date} date This accepts a js date object, this is required to be included but does not need to be populated.
    
    @example
    <h3>Calendar</h3>
  <example>
    <file name='index.html'>
      <cui-calendar date='selectedDate'></cui-calendar>
    </file>
    <file name='app.js'>
      angular.module('app', ['cui'])
        .controller('AppCtrl', function($scope) {
  ...
  module.directive('cuiCalendar', function(baseTemplatePath) {
    var Month;
    Month = (function() {
      function Month(scope) {
        var startingDay;
        this.scope = scope;
        this.viewDate = new Date(this.scope.viewDate);
        this.date = new Date(this.scope.date);
        this.cal = [];
        this.month = this.viewDate.getMonth();
        this.year = this.viewDate.getFullYear();
        this.firstDay = new Date(this.year, this.month, 1);
        startingDay = this.firstDay.getDay();
        this.offset = startingDay + 1;
        if (this.offset === 1) {
          this.offset = 8;
        }
      ...
      Month.prototype.daysInMonth = function(month, year) {
        return new Date(year, month, 0).getDate();
      };

      Month.prototype.genMonth = function() {
        var rollingWeek, week, _i;
        rollingWeek = this.firstDay;
        rollingWeek.setDate(rollingWeek.getDate() - this.offset);
        for (_i = 0; _i <= 5; _i++) {
          week = this.genWeek(rollingWeek);
          this.cal.push(week);
      ...
      Month.prototype.genWeek = function(rollingWeek) {
        var day, localWeek, ret, _i;
        this.rollingWeek = rollingWeek;
        localWeek = new Date(this.rollingWeek);
        ret = {
          days: []
        };
        for (_i = 0; _i <= 6; _i++) {
          this.rollingDayOfWeek = localWeek.setDate(localWeek.getDate() + 1);
          day = this.genDay(this.rollingDayOfWeek);
          ret.days.push(day);
        }
        return ret;
      };

      Month.prototype.genDay = function(day) {
        var localDay, ret;
        localDay = new Date(day);
        ret = {};
        ret.date = localDay;
        ret.selected = day === Date.parse(this.date);
        ret.inMonth = localDay.getMonth() === (new Date(this.viewDate)).getMonth();
        return ret;
      };
    ...
    return {
      ...
      controller: function($scope, $element, $attrs) {
        ...
        cal = new Month($scope);
        return $scope.weeks = cal.genMonth();
      },
      link: function($scope) {
        $scope.$watch('date', function() {
          if (angular.isDate($scope.date)) {
            return $scope.viewDate = $scope.date;
          }
        });
        $scope.$watch('viewDate', function() {
          var cal;
          if (angular.isDate($scope.viewDate)) {
            cal = new Month($scope);
            return $scope.weeks = cal.genMonth();
          }
        });
        $scope.clickDay = function(date) {
          return $scope.date = new Date(date);
        };
        $scope.nextMonth = function() {
          var cal;
          this.now = $scope.viewDate;
          if (this.now.getMonth() === 0) {
            $scope.viewDate = new Date(this.now.getFullYear() - 1, 11);
          } else {
            $scope.viewDate = new Date(this.now.getFullYear(), this.now.getMonth() - 1);
          }
          cal = new Month($scope);
          return $scope.weeks = cal.genMonth();
        };
        return $scope.previousMonth = function() {
          var cal;
          this.now = $scope.viewDate;
          if (this.now.getMonth() === 11) {
            $scope.viewDate = new Date(this.now.getFullYear() + 1, 0);
          } else {
            $scope.viewDate = new Date(this.now.getFullYear(), this.now.getMonth() + 1);
          }
          cal = new Month($scope);
   ...
   *# columns
  Each entry can must contain a:
  
  1. `map` specifying the key to look in the response of the server for.
  
  Additionally, each entry can contain a:
  
  2. `label` specifying the header label for the column, and
  3. `cellTemplateUrl` specifying the location of an HTML partial to fetch and compile. This can be useful for things like buttons, icons, and forms in the column. Additionally, each partial has access the the `column` and (more importantly) `row` data.
  ...
  `_query` CANNOT be overidden, and it is the only property than cannot. It is simply exposing the default behavior of the params to send to the server for your convenience. The following `query` function would work exactly the same:
    ...
    @param {array=} selected An array of the selected rows.
  ...
  <example name='dataGrid'>
    <file name='index.html'>
      {{ response.count }} total rows
      <cui-button ng-click="selected = []">Deselect {{ selected.length }} selected rows</cui-button>
      <cui-data-grid config="settings" response="response" selected="selected"></cui-data-grid>
    </file>
    <file name='app.js'>
      angular.module('app', ['cui'])
        .controller('AppCtrl', function($scope) {
          $scope.settings = {
            ...
            columns: [{
              ...
              label: 'Status',
              map: 'online',
              cellTemplateUrl: 'examples/dataGrid.cellIconTmpl.html'
            }, {
              label: 'Type',
              map: 'type'
            }, {
  ...
  module.directive('cuiDataGrid', function(baseTemplatePath, cuiDataSourceService, $parse, cuiLoading) {
    return {
      ...
      link: function(scope, tElement, tAttrs) {
        var dataSource, getPaginationRange, queryWithDefaults, updatePaginator, updateQuery, _base, _base1, _base2, _base3, _ref, _ref1;
        if (scope.config != null) {
          ...
          if ((_base3 = scope.config).totalPages == null) {
            _base3.totalPages = function(config, response) {
              return Math.ceil(response.count / config.pageSize);
            };
          }
        ...
        if ((_ref1 = scope.config) != null ? _ref1.url : void 0) {
          ...
          scope.$watch('config.url', function(newUrl, oldUrl) {
            if (newUrl !== oldUrl) {
              dataSource = cuiDataSourceService(newUrl);
            }
            return updateQuery();
          });
        ...
        scope.isSelected = function(row) {
          var rowIterator, _i, _len, _ref2;
          _ref2 = scope.selected;
          for (_i = 0, _len = _ref2.length; _i < _len; _i++) {
            rowIterator = _ref2[_i];
            if (angular.equals(row, rowIterator)) {
              return true;
            }
          }
          return false;
        };
        scope.deselect = function(row) {
          var index;
          for (index in scope.selected) {
            if (angular.equals(row, scope.selected[index])) {
  ...
  /**
    ...
    @description The Date picker uses cui-calendar and cui-textbox to make a date picking area.
    ...
    @param {Object} ngModel - Object in which to store, or set, the currently selected date
  
    @param {Date=} date - This accepts a js date object or a date string in a format recognized by the Date.parse().  This is required to be set to a variable name but you do not need to provide a value.
  
    @param {String=} dateFormat - Specify the format of the date in the input. This does not affect the returned date object to the model.
  
    @param {Boolean=} ngRequired - Expression that evaluates to true|false, or a string value of true|false,
    that defines if the input is required.
  
    @param {Boolean=} ngDisabled - Expression that evaluates to true|false, or a string value of true|false,
    that defines if the input is disabled.
  
    @example
  <example name='datePicker'>
    <file name='index.html'>
      <form novalidate name='datepickerExample' class='cui-form-stacked'>
        <div class='cui-form-group'>
          <label>Select a date</label>
          <cui-date-picker name='datepicker' date-format='M/d/yyyy' ng-model="formData.date" ng-disabled='isDisabled' ng-required='isRequired' cui-memo='datepicker.memos'></cui-date-picker>
        </div>
  
        <p>date:  {{formData.date}}</p>
        <p>datepickerExample.$error: {{datepickerExample.$error}}</p>
      </form>
      <cui-button ng-click='toggleDisabled()'>Disable Datepicker</cui-button>
      <cui-button ng-click='toggleRequired()'>Require Datepicker</cui-button>
    </file>
    <file name='app.js'>
      angular.module('app', ['cui'])
        .controller('AppCtrl', function($scope) {
          $scope.formData = [];
          // Example to set date
          // $scope.formData.date = new Date('12/12/1996')
          $scope.datepicker = []
          $scope.datepicker = {
            memos: {
  ...
  module.directive('cuiDatePicker', function(baseTemplatePath, $filter, $document) {
    return {
      templateUrl: "" + baseTemplatePath + "datePicker.html",
      restrict: 'EA',
      require: '?ngModel',
      replace: true,
      scope: {
      ...
      controller: function($scope) {
        return $scope.calendarShowing = false;
      },
      link: function(scope, iElement, iAttrs, ngModelCtrl) {
        var input, inputEl, readableDate, update, validation;
        inputEl = iElement.find('input');
        input = angular.element(inputEl);
        scope.toggleCalendar = function() {
          return scope.calendarShowing = !scope.calendarShowing;
        };
        readableDate = function(value) {
          return $filter('date')(value, iAttrs.dateFormat);
        };
        ...
        update = function() {
          ...
          if (!ngModelCtrl.$error.date) {
            date = new Date(Date.parse(inputValue));
            angular.element(input).val(readableDate(date));
        ...
        ngModelCtrl.$formatters.unshift(function(value) {
          ...
          if (!inputValue && !value) {
            ...
            angular.element(input).val(readableDate(ngModelCtrl.$viewValue));
          }
          return value;
        });
        ngModelCtrl.$formatters.push(validation);
        return ngModelCtrl.$parsers.push(function(value) {
          if (scope.calendarShowing) {
            scope.calendarShowing = !scope.calendarShowing;
          }
          return value;
        });
      }
  ...
  /**
    ...
    @description The Drop down button creates a button and menu combination. Because the Drop down button
    generates a Button, the same attributes, such as size and color, can be used. See the Button documentation
    ...
    @restrict E
    @transclude Menu items, cui-menu-item, to be listed as options in the menu, and the
    label of the Drop down button.
  
    @param {String} Label Label that should be displayed in the button.
  
    @param {String=} menuAnchor Define how the menu should anchor itself and
    in which direction the menu should open towards. `menu-anchor='[right] [left]'`. Default
    anchor position is `left` if `menuAnchor` is not set.
  ...
  <example name='regularDropDownButton'>
    <file name='index.html'>
      <cui-drop-down-button label='Drop down button' cui-type='primary' icon="dashboard">
        <cui-menu-item ng-click='click("Example 1 - Item 1")'>Item 1</cui-menu-item>
        <cui-menu-item ng-click='click("Example 1 - Item 2")'>Item 2</cui-menu-item>
        <cui-menu-item ng-click='click("Example 1 - Item 3")'>Item 3</cui-menu-item>
  ...
  <example name='rightDropDownButton'>
    <file name='index.html'>
      <cui-drop-down-button label='Drop down button anchor position' menu-anchor='right' style='float: right'>
        <cui-menu-item ng-click='click("Example 2 - Item 1")'>Item 1</cui-menu-item>
        <cui-menu-item ng-click='click("Example 2 - Item 2")'>Item 2</cui-menu-item>
        <cui-menu-item ng-click='click("Example 2 - Item 3")'>Item 3</cui-menu-item>
      </cui-drop-down-button>
    </file>
    <file name='app.js'>
      angular.module('app', ['cui'])
  ...
  <example name='disabledDropDownButton'>
    <file name='index.html'>
      <cui-drop-down-button label='Drop down button disabled' ng-disabled='true'>
        <cui-menu-item ng-click='click("Example 3 - Item 1")'>Item 1</cui-menu-item>
        <cui-menu-item ng-click='click("Example 3 - Item 2")'>Item 2</cui-menu-item>
        <cui-menu-item ng-click='click("Example 3 - Item 3")'>Item 3</cui-menu-item>
      </cui-drop-down-button>
    </file>
    <file name='app.js'>
      angular.module('app', ['cui'])
  ...
  module.directive('cuiDropDownButton', function(baseTemplatePath) {
    return {
      ...
      compile: function(element, attrs) {
        ...
        if (attrs.ngDisabled != null) {
          return button.attr('ng-disabled', 'ngDisabled');
        }
      }
    };
  });
  ...
  module.directive('cuiIcon', function(baseTemplatePath) {
    return {
  ...
  <example name='masthead'>
    <file name='index.html'>
      <cui-masthead application-name='Active System Manager' application-subname='Admin panel' show-controls=true>
        <!-- Masthead controls -->
        <cui-drop-down-button label='{{username}}'>
          <cui-menu-item ng-click='manageAccount()'>Manage Account</cui-menu-item>
          <cui-menu-item>Log out</cui-menu-item>
        </cui-drop-down-button>
  ...
  module.directive('cuiMasthead', function(baseTemplatePath, $document) {
    ...
    return {
      ...
      link: function(scope, element, attrs) {
        var drawerButton, mastheadControls, stop;
        ...
        mastheadControls.on('click', stop);
        drawerButton.on('click', stop);
  ...
  <example name='navigationList'>
    <file name='index.html'>
      <cui-navigation-list items="treeFamily" persist-as=true watch-location=true></cui-navigation-list>
    </file>
        ...
        return function(scope, iElement, iAttr, cuiNavigationListController) {
          ...
          return scope.showTab = function() {
            if ((scope.items.children != null) && scope.items.children.length > 0) {
              ...
              scope.$emit('navigationListItemClick', true);
            }
            cuiNavigationListController.selectNode(scope);
  ...
  /**
    ...
    @param {boolean=} collapsed Sets the Pane to be collapsed on render.
    @param {boolean=} collapsible Sets whether the Pane's collapsed state can be toggled by the user.
    @param {string=} icon An [Icon](../icon) which is displayed next to the `title`.
    @param {string} title The title for the Pane.
  ...
  <example>
    <file name='index.html'>
      <div class='cui-grid'>
        <cui-pane class='cui-unit cui-two-thirds' title='Profile' icon='user'>
          <div ng-include="'form.html'"></div>
        </cui-pane>
        <div class='cui-unit cui-one-third'>
          <cui-pane title='Help' class='help-pane' collapsible=false>
          ...
          <cui-pane title='New Features' icon='lightbulb' collapsed=true>
    ...
    <file name='app.js'>
      angular.module('app', ['cui'])
        .controller('AppCtrl', function($scope) {
          $scope.$on('collapsed', function(e, s) {
            console.log('collapsed', s)
          })
          $scope.$on('expanded', function(e, s) {
  ...
  module.directive('cuiPane', function(baseTemplatePath) {
    return {
      ...
      compile: function(element, attrs) {
        ...
        return function(scope, element, attrs) {
          scope.collapsed = attrs.collapsed || false;
          if (scope.collapsible == null) {
            scope.collapsible = true;
          }
          scope.icon = attrs.icon || '';
          scope.collapseIcon = scope.collapsed ? 'angle-right' : 'angle-down';
      ...
      compile: function(element, attrs) {
        ...
        return function(scope, element, attrs) {
          return scope.$watch('value', function(newValue) {
            return progressBar.attr('value', newValue);
          });
        };
      }
    };
            ...
            toolbar: function() {
              ...
              setToolTemplate = function(tool) {
                ...
                if (toolSet[tool].type === 'button') {
                  ...
                  template = "<cui-drop-down-button size='small' label='" + toolSet[tool].label + "' ng-disabled='htmlMode()'>\n  " + toolSet[tool].menuTemplate + "\n</cui-drop-down-button>";
                }
                return template;
              };
              for (tool in toolSet) {
  ...
  /**
    ...
    @param {Function=} defaultAction Function that should respond when the default action is clicked.
  
    @param {String=} icon The type of icon. See the Icon.
  
    @param {String} label The label of the button, describes the default action.
  
    @param {String=} menuAnchor Define how the menu should anchor itself and in which direction the menu should open towards. `menu-anchor='[right] [left]'`. Default anchor position is `left` if `menuAnchor` is not set.
  ...
  module.directive('cuiSplitButton', function(baseTemplatePath) {
    return {
  ...
  /**
    ...
    @param {function=} tabShown Called when the tab is clicked, or shown
  ...
  ```
    ...
    @directive cui-application-frame
    @controlType foundational
    @restrict E
  ...
  module.directive('cuiApplicationFrame', function(baseTemplatePath) {
    return {
      ...
      link: function(scope) {
        ...
        return scope.$on('navigationListItemClick', function() {
  ...
  /**
    ...
    @param {Object=} startScreenSettings Object that you can set or read current start screen settings or variables. The object has the following properties:
   ...
   * * __messageText__ &mdash; `{string}` &mdash;
   *   Displays this text and html in the message area. Currently does not support any cui controls.  If text is populated it will display, to hide set messageText = undefined
   * * __messageIcon__ &mdash; `{string}` &mdash;
   *   Name of the icon you want displayed.  No Default is warning-sign
   * * __messageIconColor__ &mdash; `{string}` &mdash;
   *   Color of the icon you want displayed.  Default is yellow
  
  
    @param {Function=} onSignIn A function passed the object `userData` as its first parameter containing `username`, `password`, `domain`, `rememberMe`, `parsedUsername`, and `parsedDomain` properties.  `parsedUsername`, and `parsedDomain` trigger on the @ or backslash in the username field.
  ```html
  ...
  <example name='startScreen'>
    <file name='index.html'>
      <cui-start-screen
        start-screen-settings="settings"
    ...
    <file name='app.js'>
      angular.module('app', ['cui'])
        .controller('AppCtrl', function($scope) {
          // Example of Settings for Start Screen, to actually change anything from the defaults you must use start-screen-settings.
          ...
          $scope.signIn = function(userData){
            // Example of using a function instead of watching for the alert.
  
            // Currently only supports text and html, no cui elements.
            $scope.settings.messageText = "Logged in with " + "Domain: " + userData.domain + " Username: " + userData.username + " Password: " + userData.password + " Remember Me: " + userData.rememberMe;
  ...
  module = angular.module('cui.modules.startScreen', ['cui.base']);

  module.directive('cuiStartScreen', function(baseTemplatePath, $sce, $timeout) {
    return {
      ...
      link: function(scope, element, attrs) {
        ...
        if (scope.startScreenSettings == null) {
          scope.startScreenSettings = {};
        }
        if ((_base = scope.startScreenSettings).applicationName == null) {
          _base.applicationName = 'Application Name';
        }
        if ((_base1 = scope.startScreenSettings).signInLabel == null) {
          _base1.signInLabel = 'Sign In';
        }
        if ((_base2 = scope.startScreenSettings).isSpinning == null) {
          _base2.isSpinning = false;
        }
        if ((_base3 = scope.startScreenSettings).showDomain == null) {
          _base3.showDomain = false;
        }
        if ((_base4 = scope.startScreenSettings).showMessage == null) {
          _base4.showMessage = false;
        }
        if ((_base5 = scope.startScreenSettings).showRememberMe == null) {
          _base5.showRememberMe = false;
        }
        if ((_base6 = scope.startScreenSettings).rememberMe == null) {
          _base6.rememberMe = false;
        }
        if ((_base7 = scope.startScreenSettings).domainPlaceholder == null) {
          _base7.domainPlaceholder = 'Domain';
        }
        if ((_base8 = scope.startScreenSettings).usernamePlaceholder == null) {
          _base8.usernamePlaceholder = 'Username';
        }
        if ((_base9 = scope.startScreenSettings).passwordPlaceholder == null) {
          _base9.passwordPlaceholder = 'Password';
        }
        if ((_base10 = scope.startScreenSettings).copyright == null) {
          _base10.copyright = $sce.trustAsHtml('\xA9 2014 Dell Inc. ALL RIGHTS RESERVED');
        }
        if ((_base11 = scope.startScreenSettings).rememberMeText == null) {
          _base11.rememberMeText = 'Remember Me';
        }
        scope.handleSignIn = function() {
          var data, temp;
          data = {
            domain: scope.startScreenSettings.domain,
            username: scope.startScreenSettings.username,
            password: scope.startScreenSettings.password,
            rememberMe: scope.startScreenSettings.rememberMe
          };
          if (scope.startScreenSettings.username.indexOf('\\') > -1) {
            temp = scope.startScreenSettings.username.split('\\');
            data.parsedDomain = temp[0];
            data.parsedUsername = temp[1];
          } else if (scope.startScreenSettings.username.indexOf('@') > -1) {
            temp = scope.startScreenSettings.username.split('@');
            data.parsedDomain = temp[1];
            data.parsedUsername = temp[0];
          } else {
            data.parsedDomain = null;
            data.parsedUsername = null;
          }
          scope.$emit('cuiStartScreen:signIn', data);
          return scope.onSignIn({
            userData: data
          });
        };
        return scope.$watch('startScreenSettings.messageText', function(msg) {
          return scope.trustedHtml = $sce.trustAsHtml(msg);
        });
      }
    };
  });
  ...
  Please note that while these will throw if out-of-bounds or the title of the step isn't found, they **will** allow selecting steps that are invalid.
    ...
    ** `'cui:wizard:cancel'`: Cancels (emits `'cui:wizard:cancelled'` and goes to the first step).
    ** `'cui:wizard:finish'`: Finishes (emits `'cui:wizard:finished'` and goes to the last step).
  ...
  <example name='wizard'>
    <file name='index.html'>
      ...
      <cui-wizard edit-mode='editMode'>
        <cui-wizard-step title="Welcome!" invalid='!choices.branch'>
        ...
        <cui-wizard-step ng-repeat='step in steps' title="{{step.title}}" ng-if='choices.branch === "repeat"'>
          <div ng-include='step.src'></div>
        </cui-wizard-step>
        <div ng-if='choices.branch === "static"'>
          <cui-wizard-step title='Steps added conditionally'>
            <p>This is an example of adding steps conditionally.</p>
          </cui-wizard-step>
          <cui-wizard-step title='More info'>
            <p>This is an example of adding steps conditionally, with some more information.</p>
          </cui-wizard-step>
        </div>
        <cui-wizard-step title='Finish'>
          Finish your wizard.
        </cui-wizard-step>
      </cui-wizard>
    </file>
  ...
  <example name='wizard'>
    <file name='index.html'>
      <cui-wizard>
        <cui-wizard-step title="Introduction">
          <h1>Start the interactive wizard</h1>
        ...
        <cui-wizard-step title='First' on-next='first.fn()'>
        ...
        <cui-wizard-step title='Second' on-next='second.fn()' on-enter='stepEntered("Second")'>
          <p>A function that returns a Promise</p>
        ...
        <cui-wizard-step title='Third' on-next='third.fn()'>
          <p>A function that returns a Promise</p>
  ...
  <example name='wizardInModal'>
    ...
    <file name='wizard.html'>
      <cui-wizard edit-mode='editMode'>
        <cui-wizard-step title="Welcome, this is a really long title for a wizard step.">
        ...
        <cui-wizard-step title="ToS" invalid='!wiz.acceptedTerms'>
      ...
      <form name="form" ng-submit='next(form.$invalid)'>
        <cui-wizard-step title="Register an account" invalid='form.$invalid'>
          <h1>Continuing</h1>
        ...
        <cui-wizard-step title="More steps 2">
          <p>Even more steps!!</p>
        </cui-wizard-step>
        <cui-wizard-step title="More steps 3">
          <p>Even more steps!!</p>
        </cui-wizard-step>
      </cui-wizard>
    </file>
    ...
    return {
      ...
      controller: function($scope, $element) {
        ...
        this.updateSteps = (function(_this) {
          return function() {
            var scope, step, stepElements, _i, _len;
        ...
        $scope.$watch('currentStepIndex', function(index, oldIndex) {
          var _ref, _ref1, _ref2, _ref3;
          if ((_ref = steps[index]) != null) {
            _ref.touched = true;
          }
          $scope.currentStep = (_ref1 = steps[index]) != null ? _ref1.title : void 0;
          if ((_ref2 = steps[index]) != null) {
            _ref2.onEnter();
          }
          if (index !== oldIndex) {
            return $scope.$emit('cui:wizard:stepChanged', {
              ...
              index: oldIndex,
              title: (_ref3 = steps[oldIndex]) != null ? _ref3.title : void 0
            });
          }
        });
        ...
        $scope.canFinish = function() {
          var _ref, _ref1;
          if ($scope.editMode) {
            return (_ref = steps[steps.length - 1]) != null ? _ref.forward : void 0;
          } else {
            return !((_ref1 = steps[$scope.currentStepIndex]) != null ? _ref1.invalid : void 0) && $scope.currentStepIndex === steps.length - 1;
          }
        };
        this.goto = function(identifier) {
          ...
          if (angular.isString(identifier)) {
            ...
            return $scope.currentStepIndex = identifier;
          } else {
  ...
  /**
    ...
    @param {string} title The title of the step (to be displayed on the left hand side of the wizard).
  ...
  module.directive('cuiWizardStep', function(baseTemplatePath) {
    return {
  ...
  <blockquote>You must use the application frame to use the alert service at this time.</blockquote>
  ...
  To remove all alerts, simply call `cuiAlertService.flush()`.
  
  > If there are more than three alerts on the page, the alert service will automatically remove the oldest.
  
  @returns {promise} The service returns a `promise` that provides the element on success. Usage:
  
  ```javascript
  cuiAlertService('Hello, world!').then(function(el) {
    // Do something, maybe el.remove()?
  });
  ```
  ...
  <example name='alertService'>
    <file name='index.html'>
      <cui-application-frame application-name="Alert Service"
        ...
        <cui-button ng-click="alert()" cui-type="primary">Push default alert</cui-button>
  ...
  module.factory('cuiAlertService', function($document, $timeout, $q, $rootScope, $sce, $compile, $templateCache, baseTemplatePath) {
    ...
    alertService.flush = function() {
      var elLocation;
      elLocation = getAlertsLocation();
      return angular.forEach(elLocation.children(), function(child) {
        return angular.element(child).isolateScope().close();
      });
    };
    getAlertsLocation = function(name) {
      var alertPlaceholder;
      alertPlaceholder = angular.element($document[0].querySelector('[cui-alert-service]'));
      if (alertPlaceholder.length === 0) {
        throw new Error("Couldn't find application frame or [cui-alert-service] on the page!");
      }
    ...
    setAlerts = function(args, type, deferred) {
      ...
      if (elLocation.children().length > 3) {
        angular.element(elLocation.children()[0]).isolateScope().close();
      }
  ...
  The second parameter to the `cuiDataSourceService` function is a configuration object. This object allows you to set default query parameters, headers for the request, and the `id` field of single object.
   ...
   * Methods available on a DataSource object
   ...
   * Methods on single record objects
  ...
  module.provider('cuiLoading', function() {
    ...
    this.$get = (function(_this) {
      return function(baseTemplatePath, cuiModal, cuiLoadingPromise) {
...
$templateCache.put("__cui/button-link.html","<button ng-transclude class=cui-button-link ng-mouseup=blur()></button>");
$templateCache.put("__cui/button.html","<button class=cui-button ng-class=\"\'cui-button-type-\'+cuiType\" ng-mouseup=blur()><div class=cui-button-overlay ng-transclude></div></button>");
$templateCache.put("__cui/calendar.html","<div class=cui-calendar><div class=cui-calendar-header><span class=cui-left-arrow ng-click=nextMonth()><cui-icon icon=angle-left></cui-icon></span> {{ viewDate | date:\'MMMM yyyy\' }} <span class=cui-right-arrow ng-click=previousMonth()><cui-icon icon=angle-right></cui-icon></span></div><div class=cui-month><div class=\"cui-day-of-week cui-week\" ng-repeat=\"week in weeks | limitTo:1\"><div class=cui-short ng-repeat=\"day in week.days\">{{ day.date | date:\'EEE\' }}</div></div><div class=cui-week ng-repeat=\"week in weeks\"><div class=cui-day data-date=\"{{day.date | date:\'shortDate\'}}\" ng-repeat=\"day in week.days\" ng-click=clickDay(day.date) ng-class=\"{&quot;selected&quot;: day.selected,&quot;not-in-month&quot;: !day.inMonth}\">{{ day.date | date:\'d\' }}</div></div></div><div class=cui-calendar-footer ng-click=clickDay(today)>{{ today | date:\'fullDate\' }}</div></div>");
...
$templateCache.put("__cui/dataGrid.html","<div class=\"cui-data-grid cui-table\"><table><thead><tr><td ng-if=\"config.searchable !== false\" colspan={{config.columns.length}}><cui-textbox ng-model=config.searchQuery placeholder=\"{{ \'CUI_DATAGRID_SEARCH_PLACEHOLDER\' | translate}}\"></cui-textbox></td></tr><tr><th ng-repeat=\"column in config.columns\" ng-click=toggleSort(column)><span ng-class=\"\n            { \'sort-ascent\': config.sortCol === column.map && config.sortDir === \'asc\',\n             \'sort-descent\': config.sortCol === column.map && config.sortDir === \'desc\'}\">{{column.label || column.map}}</span></th></tr></thead><tbody><tr ng-repeat=\"row in dataGetter(response)\" ng-class=\"{selected: isSelected(row)}\" ng-click=\"isSelected(row) ? deselect(row) : selected.push(row)\"><td ng-repeat=\"column in config.columns\" class=cui-data-grid-cell>{{ row[column.map] }}</td></tr></tbody><tfoot><tr ng-if=\"dataGetter(response).length === 0\"><td colspan={{config.columns.length}} class=no-rows translate=CUI_DATAGRID_NO_DATA></td></tr><tr><td colspan={{config.columns.length}}><div class=pagination ng-if=showPaginator><ul><li ng-if=\"pages[0].number !== 1\" ng-click=\"config.currentPageNumber = config.currentPageNumber - 1\">&lt;</li><li ng-repeat=\"page in pages\" ng-class=\"{active: config.currentPageNumber === page.number}\" ng-click=\"config.currentPageNumber = page.number\"><a>{{page.number}}</a></li><li ng-if=\"pages[pages.length-1].number !== totalPages\" ng-click=\"config.currentPageNumber = config.currentPageNumber + 1\">&gt;</li></ul></div></td></tr></tfoot></table></div>");
$templateCache.put("__cui/datePicker.html","<span class=cui-date-picker ng-class=invalid><input class=cui-textbox placeholder=\"Enter date (m/d/yyyy)\" ng-disabled=ngDisabled ng-required=ngRequired><cui-button ng-click=toggleCalendar() ng-disabled=ngDisabled type=button><cui-icon icon=calendar></cui-icon></cui-button><cui-calendar date=ngModel ng-show=calendarShowing></cui-calendar></span>");
$templateCache.put("__cui/dropDownButton.html","<div class=cui-drop-down-button><cui-button cui-type={{cuiType}} cui-menu-toggle><cui-icon ng-if=icon class=cui-drop-down-button-icon icon={{icon}}></cui-icon>{{label}}<cui-icon icon=caret-down></cui-icon></cui-button><div class=\"cui-menu-wrap cui-menu-anchor-{{menuAnchor}}\"><div cui-menu ng-transclude></div></div></div>");
$templateCache.put("__cui/dropDownList.html","<select class=cui-drop-down-list ng-transclude></select>");
$templateCache.put("__cui/icon.html","<i class=\"cui-icon cui-icon-{{icon}} cui-icon-color-{{color}}\" ng-style=\"{&quot;font-size&quot;: size}\"></i>");
...
$templateCache.put("__cui/navigationListItem.html","<a ng-click=showTab() ng-class=\"{ \'cui-navigation-toplevel\': toplevel,\r\n               \'cui-navigation-secondary\': items.visible,\r\n               \'cui-navigation-active\': items.active }\" ng-href={{items.href}} href class=cui-navigation-link><cui-icon icon=caret-right ng-class=\"{ \'cui-navigation-open\': items.visible,\r\n                        \'cui-navigation-invisible\': !items.children.length }\" class=cui-navigation-collapse></cui-icon>&nbsp;<cui-icon icon={{items.icon}} ng-if=items.icon class=cui-navigation-icon></cui-icon>&nbsp;{{ items.label }}</a><div ng-show=items.visible aria-hidden={{!items.visible}}><ul class=cui-navigation-group ng-class=\"{\'cui-navigation-secondary\': items.visible,\'cui-navigation-haschildren\': items.children.length > 0}\" role=menu><li ng-repeat=\"child in items.children\" role=menuitem aria-haspopup=\"{{child.children.length > 0}}\"><cui-navigation-list-item items=child></cui-navigation-list-item></li></ul></div>");
$templateCache.put("__cui/pane.html","<div class=cui-pane ng-class=\"{&quot;collapse-horizontal&quot;: collapseOrientation == &quot;horizontal&quot;, &quot;collapsed&quot;: collapsed}\" ng-mouseleave=mouseLeft()><h2 class=cui-pane--title-bar ng-class=\"{&quot;collapsible&quot;: collapsible}\" ng-click=toggleCollapse()><span class=cui-pane--title-bar--icon ng-if=icon><cui-icon icon={{icon}}></cui-icon></span> <span class=cui-pane--title-bar--title>{{title}}</span> <span class=cui-pane--collapse-icon ng-if=collapsible><cui-icon icon={{collapseIcon}}></cui-icon></span></h2><p class=cui-pane--content ng-transclude ng-show=!collapsed></p></div>");
$templateCache.put("__cui/progressBar.html","<div class=cui-progress-bar><progress max=100 title=\"{{value || &quot;Unknown&quot;}} percent complete\" class=cui-progress-bar-type-{{cuiType}}></progress></div>");
...
$templateCache.put("__cui/applicationFrame.html","<div class=cui-application-frame ng-class=\"{&quot;cui-navigation-showing&quot;: navigationShowing}\" ng-swipe-left=swipeLeft()><cui-masthead fixed=true show-navigation-drawer=\"{{navigationItems !== undefined}}\" show-controls=mastheadShowControls application-name={{applicationName}} application-subname={{applicationSubname}} application-href={{applicationHref}} application-logo={{applicationLogo}}><span cui-application-frame-transclude=cui-controls></span></cui-masthead><cui-navigation-list ng-if=\"navigationItems !== undefined\" items=navigationItems persist-as={{persistAs}} watch-location=true class=cui-left-navigation></cui-navigation-list><div class=cui-container ng-swipe-right=swipeRight() ng-click=swipeLeft() ng-class=\"{&quot;cui-navigation-none&quot;: navigationItems === undefined}\"><div cui-alert-service></div><div class=cui-content cui-application-frame-transclude=cui-content></div></div></div>");
$templateCache.put("__cui/startScreen.html","<div class=\"cui-start-screen cui-theme-blue\"><div class=cui-container><cui-icon icon=dell-halo class=dell-logo></cui-icon><h1 class=cui-application-name>{{startScreenSettings.applicationName}}</h1><div class=cui-message-text ng-show=trustedHtml role=alertdialog aria-labelledby=cui-alert-dialog><cui-icon icon={{startScreenSettings.messageIcon}} color={{startScreenSettings.messageIconColor}}></cui-icon>&nbsp;<span ng-bind-html=trustedHtml id=cui-alert-dialog></span></div><form name=cuiSignIn class=cui-sign-in novalidate><div class=\"cui-login-container cui-grid\" ng-hide=startScreenSettings.isSpinning><div class=\"cui-inputs login cui-unit cui-half\" ng-hide=isHidingLogin><cui-textbox name=cuiUsername class=\"tb-container cui-username\" ng-model=startScreenSettings.username placeholder={{startScreenSettings.usernamePlaceholder}} type=text required ng-attr-autofocus=true></cui-textbox></div><div class=\"cui-inputs login cui-unit cui-half\" ng-hide=isHidingLogin><cui-textbox name=cuiPassword class=\"tb-container cui-password\" ng-model=startScreenSettings.password placeholder={{startScreenSettings.passwordPlaceholder}} type=password required></cui-textbox></div><div class=\"cui-inputs cui-unit cui-half\" ng-show=startScreenSettings.showDomain><cui-textbox name=cuiDomain class=\"tb-container cui-domain\" ng-model=startScreenSettings.domain placeholder={{startScreenSettings.domainPlaceholder}} ng-required=startScreenSettings.showDomain></cui-textbox></div><div class=\"cui-inputs cui-unit cui-half\" ng-show=startScreenSettings.showRememberMe><cui-checkbox class=remember-me name=rememberMe value=startScreenSettings.rememberMe ng-model=$parent.startScreenSettings.rememberMe>{{startScreenSettings.rememberMeText}}</cui-checkbox></div><div class=cui-buttons><cui-button class=cui-sign-in-button size=medium ng-click=handleSignIn() ng-disabled=cuiSignIn.$invalid>{{startScreenSettings.signInLabel}}</cui-button></div></div><cui-spinner class=cui-spinner-container color=white size=48px ng-show=startScreenSettings.isSpinning></cui-spinner></form><div class=cui-push></div></div><div class=cui-copyright>{{startScreenSettings.copyright}}</div></div>");
$templateCache.put("__cui/wizard.html","<div class=cui-wizard><div class=cui-wizard-step-buttons><ul><li ng-repeat=\"step in steps\" ng-click=selectStepIndex($index) ng-attr-title={{step.title}} ng-class=\"{&quot;cui-wizard-active&quot;: currentStepIndex === $index,\n                     &quot;cui-wizard-disabled&quot;: isStepDisabled($index)}\"><cui-icon icon=angle-right size=20px ng-if=\"currentStepIndex === $index\" class=cui-wizard-angle></cui-icon><span class=cui-wizard-step-title>{{step.title}}</span><div class=cui-wizard-check ng-if=\"(currentStepIndex > $index || editMode) && !step.invalid && !step.onNextFailed\">&#10003;</div><div class=cui-wizard-x ng-if=\"((currentStepIndex >= $index || editMode) && step.invalid) || step.onNextFailed\">&#x2717;</div></li></ul></div><div class=cui-wizard-content><div class=cui-wizard-transclude ng-transclude></div><div class=cui-wizard-navigation><span class=cui-wizard-step-count translate=CUI_WIZARD_STEP translate-values=\"{current: currentStepIndex + 1, total: steps.length}\"></span><div class=cui-wizard-nav-buttons><cui-button ng-click=previous($event) ng-disabled=\"currentStepIndex === 0\" class=\"cui-wizard-nav-button cui-wizard-nav-previous\">{{\'CUI_WIZARD_BACK\' | translate}}</cui-button><cui-button ng-click=next($event) cui-type=primary ng-disabled=\"steps[currentStepIndex].invalid || currentStepIndex === steps.length - 1\" class=\"cui-wizard-nav-button cui-wizard-nav-next\"><span ng-if=!steps[currentStepIndex].onNextExists()>{{\'CUI_WIZARD_NEXT\' | translate}}</span> <span ng-if=steps[currentStepIndex].onNextExists()>{{\'CUI_WIZARD_SAVE_AND_NEXT\' | translate}}</span></cui-button><cui-button ng-click=finish($event) cui-type=primary ng-disabled=!canFinish() class=\"cui-wizard-nav-button cui-wizard-nav-finish\">{{\'CUI_WIZARD_FINISH\' | translate}}</cui-button><div class=cui-wizard-button-divider></div><cui-button ng-click=cancel($event) class=\"cui-wizard-nav-button cui-wizard-nav-cancel\">{{\'CUI_WIZARD_CANCEL\' | translate}}</cui-button></div></div></div></div>");
...
$templateCache.put("__cui/modal.html","<div class=cui-modal-wrapper><div ng-if=\"modal.backdrop && modal.visible\" class=cui-modal-backdrop ng-click=\"!modal.static && modal.hide()\"></div><div ng-class=modal.classes ng-include=modal.typeTemplateUrl></div></div>");}]);